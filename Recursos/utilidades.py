...
def ingresoNro(start:int=-inf, end:int=inf)->int:
    """
    Args:
        start (int, optional): Se solicitará un valor entero como mínimo. Por defecto -inf.
        end (int, optional): Se solicitará un valor entero que como máximo. Por defecto inf.
    ...
    Returns:
        int: Un número entero comprendido entre los límites indicados.
    """
    try:
...
def ingresoFlotante(start:float=-inf, end:float=inf) -> float:
    """
    Args:
        start (float, optional): Se solicitará un valor flotante como mínimo. Por defecto -inf.
        end (float, optional): Se solicitará un valor flotante que como máximo. Por defecto inf.

    Raises:
        ValueError: Se lanzará al ingresar un cáracter inválido o un número que exceda los límites comprendidos. Es gestionado por la misma función

    Returns:
        float: Un número flotante comprendido entre los límites indicados.
    """
    try:
...
def ingresarSerieNumeros(cant: int, start=-inf, end=inf) -> tuple:
    ...
    Returns:
        tuple: Una tupla conformada por n números flotantes.
    """
    nros = ()
...
def ingresoOpcion(validRange: tuple):
    ...
    try:
        ...
        """
        Si todas las opciones en validRange son enteros, 
        se asume que se espera que la opción ingresada por el usuario también lo sea,
        por lo que se la convierte.
        """
        if all(isinstance(x, int) for x in validRange):
            opcion = int(opcion)
        
        """
        Si no se ha ingresado una opción que se encuentre en validRange, 
        se la entiende por inválida y se arroja una excepción.
        """
        ...
        """
        De hallar una opción inválida, la función se convierte en una recursiva,
        de modo que la ejecución de esta no se detiene hasta ingresar una opción válida.
        """
